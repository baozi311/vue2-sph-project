  ...
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <button @click="getCode" style="height: 38px">发送验证码</button>
        <span class="error-msg">错误提示信息</span>
      </div>
      <div class="content">
        <label>登录密码:</label>
...
export default {
  ...
  methods: {
    async getCode() {
      if (this.phone) {
        try {
          const code = await this.$store.dispatch("user/getCode", this.phone);
          this.code = code;
        } catch (error) {
    ...
    async register() {
      let { phone, code, password, password2, isChecked } = this;
      if (
        phone &&
        code &&
        password &&
